...
final class IntegrationController extends Controller
{
    ...
    public function index(): Response
    {
        ...
        $syncLogs = $this->mapSyncLogs(excludeSource: 'appsolar');
        $lastSync = $syncLogs->first();

        $stats = [
            'total_syncs' => DB::table('sync_logs')->where('source', '!=', 'appsolar')->count(),
            'success_rate' => DB::table('sync_logs')->where('source', '!=', 'appsolar')->count() > 0
                ? round(DB::table('sync_logs')->where('source', '!=', 'appsolar')->where('status', 'success')->count() / DB::table('sync_logs')->where('source', '!=', 'appsolar')->count() * 100)
                : 0,
        ...
        $appsolarConfig = [
            'base_url' => config('services.appsolar.base_url', ''),
            'token_set' => ! empty(config('services.appsolar.token', '')),
            'timeout' => (int) config('services.appsolar.timeout', 30),
            'sync_enabled' => (bool) config('services.appsolar.enabled', false),
            'full_sync_schedule' => 'Diária às 02:00',
            'incremental_schedule' => 'A cada hora',
        ];

        $appsolarSyncLogs = $this->mapSyncLogs(onlySource: 'appsolar');
        $lastAppSolarSync = $appsolarSyncLogs->first();
        ...
        $appsolarStats = [
            ...
            'success_rate' => DB::table('sync_logs')->where('source', 'appsolar')->count() > 0
                ? round(DB::table('sync_logs')->where('source', 'appsolar')->where('status', 'success')->count() / DB::table('sync_logs')->where('source', 'appsolar')->count() * 100)
                : 0,
            'last_sync_at' => $lastAppSolarSync['started_at'] ?? null,
            'last_status' => $lastAppSolarSync['status'] ?? null,
            'kits_synced' => DB::table('solar_kit_specifications')->count(),
        ];
        ...
        return Inertia::render('Admin/Integration/Index', [
            ...
            'appsolarSyncLogs' => $appsolarSyncLogs,
    ...
    private function mapSyncLogs(?string $onlySource = null, ?string $excludeSource = null): Collection
    {
        $query = DB::table('sync_logs')->orderByDesc('started_at')->limit(15);

        if ($onlySource !== null) {
            $query->where('source', $onlySource);
        }

        if ($excludeSource !== null) {
            $query->where('source', '!=', $excludeSource);
        }
        ...
        return $query->get()->map(fn ($log) => [
            ...
            'archived_items' => $log->archived_items ?? 0,
            'notes' => $log->notes,
            'errors' => $log->errors ? json_decode($log->errors, true) : null,
            'started_at' => $log->started_at ? Carbon::parse($log->started_at)->format('d/m/Y H:i:s') : null,
            'finished_at' => $log->finished_at ? Carbon::parse($log->finished_at)->format('d/m/Y H:i:s') : null,
            'duration_s' => ($log->started_at && $log->finished_at)
                ? Carbon::parse($log->finished_at)->diffInSeconds(Carbon::parse($log->started_at))
                : null,
        ]);
    }
    ...
    public function runAppSolarSync(Request $request): JsonResponse
    {
        if (empty(config('services.appsolar.base_url')) || empty(config('services.appsolar.token'))) {
            return response()->json([
                'success' => false,
                'message' => 'Configure APPSOLAR_API_BASE_URL e APPSOLAR_API_TOKEN no arquivo .env antes de sincronizar.',
            ]);
        }

        try {
            $full = $request->boolean('full');
            $results = $full ? $this->appSolarSyncService->syncFull() : $this->appSolarSyncService->syncIncremental();

            return response()->json([
                'success' => true,
                'message' => "Sincronização concluída: {$results['total']} produtos processados ({$results['created']} criados, {$results['updated']} atualizados, {$results['archived']} arquivados, {$results['errors']} erros).",
                'results' => $results,
            ]);
        } catch (\Throwable $e) {
            return response()->json([
                'success' => false,
                'message' => 'Erro ao sincronizar com o AppSolar: '.$e->getMessage(),
            ]);
        }
    }
